...
import { Search, RotateCcw, SlidersHorizontal, X, Lock, ArrowRight } from 'lucide-react'
...
export function PublicJobBoard({ initialJobs }: { initialJobs: JobPosting[] }) {
    ...
    const filteredJobs = useMemo(() => {
        return standardJobs.filter((job) => {
            if (query.trim()) {
                ...
                if (!job.job_title.toLowerCase().includes(q) && !job.company_name.toLowerCase().includes(q)) return false
            }
        ...
        <div className="flex flex-col md:flex-row gap-8 lg:gap-16 items-start w-full relative justify-between">
            ...
            <div className="w-full max-w-[850px] shrink-0 order-2 md:order-1 transition-all duration-300 ease-in-out">
                <div className="mb-6 flex items-center justify-between">
                    ...
                    <Button
                        ...
                        onClick={() => { setIsFilterOpen(!isFilterOpen); setPromptOpen(false) }}
                ...
                {filteredJobs.length === 0 ? (
                    <div className="text-center py-16 bg-white rounded-xl border border-dashed border-neutral-300 shadow-sm">
                        <p className="text-neutral-500 font-medium">No jobs matched your filter combination.</p>
                        {hasActiveFilters && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {filteredJobs.map((job) => (
                            <div
                                ...
                                className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6 flex flex-col h-full hover:shadow-md hover:border-primary-300 hover:ring-2 ring-primary-500/10 cursor-pointer transition-all relative group"
                            >
                                {/* Hover lock hint */}
                                <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div className="bg-primary-50 border border-primary-100 rounded-full p-1">
            ...
            <aside
                className={`order-1 md:order-2 flex-shrink-0 transition-all duration-300 ease-in-out ${
                    (isFilterOpen || promptOpen) ? 'w-full md:w-[450px] opacity-100 translate-x-0' : 'w-0 opacity-0 overflow-hidden translate-x-12 h-0 md:h-auto'
                }`}
            >
                    ...
                    {promptOpen ? (
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center justify-end">
                                <button
                                    onClick={() => { setPromptOpen(false); setIsFilterOpen(true) }}
                            ...
                            <div className="text-center pt-4 pb-2">
                                ...
                                <h2 className="text-xl font-black text-neutral-900 tracking-tight mb-2">Sign in to view & apply</h2>
                                <p className="text-sm text-neutral-500 font-medium leading-relaxed max-w-xs mx-auto">
                                    Create a free account or log in to see full job details, save listings, and submit applications.
                                </p>
                            </div>
                            ...
                            <div className="flex flex-col gap-3 pt-2">
                                <Button
                                    ...
                                    className="w-full h-12 font-bold bg-primary-600 hover:bg-primary-700 shadow-sm transition-colors text-white text-base"
                                >
                                    Create Free Account
                                ...
                                <Button
                                    ...
                                    className="w-full h-12 font-semibold border-neutral-300 text-neutral-700 hover:bg-neutral-50 transition-colors"
                                >
                                    Sign In
                                </Button>
                            </div>
